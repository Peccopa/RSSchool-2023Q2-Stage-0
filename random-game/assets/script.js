...
const loadPage = document.querySelector('.page');
  ...
  setInterval(() => {
    ...
    if (rocks) {
      for (let i = 0; i < rocks.length; i++) {
        let rockFall = parseInt(
          window.getComputedStyle(rocks[i]).getPropertyValue('top')
        );
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.key == ' ' && !rocket) {
      ...
      setInterval(() => {
        ...
        let rocketPosition = parseInt(
          window.getComputedStyle(rocket).getPropertyValue('bottom')
        );
  ...
  setInterval(() => {
    ...
    if (rocket) {
      ...
      for (let i = 0; i < rocks.length; i++) {
        ...
        if (
          rocketBound.left + 9 >= rockBound.left &&
          ...
          blast.style.top =
            Number(
              rocks[i].style.top.substring(0, rocks[i].style.top.length - 2)
            ) -
            speedFall +
            'px';
          blast.style.bottom = rocks[i].style.bottom;
          blast.classList.add('blast');
          const audioBlast = new Audio(
            `assets/sounds/exp-${randomInt(1, 3)}.mp3`
          );
          audioBlast.play();
          rocks[i].parentElement.removeChild(rocks[i]);
          setTimeout(() => {
